...
window.onload = function () {
    ...
    function initTexture() {
        ...
        nimage.src = '../../img/normalmap.png';
        nimage.onload = function () {
            var ntexture = gl.createTexture();
            gl.uniform1i(gl.getUniformLocation(gl.program, "texNimage"), 0);
            gl.activeTexture(gl.TEXTURE0);
            gl.bindTexture(gl.TEXTURE_2D, ntexture);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, nimage);
    ...
    var Vvalues;
    Vvalues = mat4();
    Vvalues = lookAt(eye, vec3(0.0, 0.0, -1.0), vec3(0.0, 1.0, 0.0));
    ...
    function tick() {
        ...
        //Mvalues = rotateY(a);
        eye = vec3(r * Math.sin(a), 0.0, r * Math.cos(a));
        // gl.uniform3fv(gl.getUniformLocation(gl.program, "eye"), flatten(eye));
        // Vvalues = lookAt(eye,vec3(0, 0, 0),vec3(0, 1, 0));
        //gl.uniformMatrix4fv(Mmatrix, false, flatten(Mvalues));
        //gl.uniformMatrix4fv(Vmatrix, false, flatten(Vvalues));
        render();
        if (play) {
